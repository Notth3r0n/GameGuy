...
class HelpCommand(commands.Cog):
  ...
  async def help(self,ctx, cat: str=None):
    ...
    if cat == None:
      ...
      cat_em.add_field(name='Mod', value='Do !help mod for a list of moderation commands(only of mods)')
    ...
    #output when shop cat is specified
      ...
      await ctx.reply(embed=store_embed)
    ...
    #output if fish cat is specified
      ...
      await ctx.reply(embed=fish_em)

    #output if mod car is specified
    if ctx.author.guild_permissions.administrator == True and cat.lower() == "mod":
      mod_em = discord.Embed(title="Moderator Commands List", color=discord.Color.blurple())
      mod_em.add_field(name="Warn [user]", value="Warn user", inline=False)
      mod_em.add_field(name="Warnings [user]", value="Check number of warnings user has", inline=False)
      mod_em.add_field(name="Kick [user]", value="Kick user", inline=False)
      mod_em.add_field(name="Ban [user]", value="Ban user", inline=False)
      mod_em.add_field(name="Unban [user]", value="Unban user", inline=False)
      await ctx.send(embed=mod_em)
    elif ctx.author.guild_permissions.administrator == False and cat.lower() == "mod":
        fmod_em = discord.Embed(title="Invaid Command", description="You do not have permission to use this command!", color=discord.Color.blurple())
        await ctx.reply(embed=fmod_em)
