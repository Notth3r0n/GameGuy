...
class BlackjackGame(commands.Cog):
    ...
    async def blackjack(self, ctx, amount: int):
        ...
        if amount <= 0:
          ...
          await ctx.reply(embed=amount_em)
        ...
        await ctx.reply(embed=player_em)
        ...
        while self.calculate_hand_value(player_hand) < BLACKJACK:
            ...
            await ctx.reply(embed=hitstand_em)
            try:
        ...
        if player_value > BLACKJACK:
            ...
            await ctx.reply(embed=dealer_bust_em)
        ...
        await ctx.reply(embed=player_hand_em)
