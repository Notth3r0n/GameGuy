...
class TicTacToe(commands.Cog):
    ...
    async def tictactoe(self, ctx, opponent: str = "bot"):
        ...
        if opponent.lower() not in ["bot", "user"]:
            await ctx.reply("Invalid opponent choice. Please choose 'bot' or 'user'.")
            return
        ...
        while not winner and EMPTY in board:
            ...
            if board[move] == EMPTY:
                ...
                if current_player == PLAYER_X:
                    await ctx.reply("That spot is already taken. Try again.")
        ...
        if winner:
            if winner == PLAYER_X:
                if opponent == "bot":
                    await ctx.reply("You win! 5 coins have been credited")
                ...
                if opponent == "bot":
                    await ctx.reply("Bot wins!")
                else:
                    await ctx.reply(f"{opponent.mention} wins! 5 coins hae been credited")
                    with open("cogs/eco.json", "r") as f:
                        user_eco = json.load(f)
