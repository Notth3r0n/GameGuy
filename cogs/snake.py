...
class SnakeGame(commands.Cog):
    ...
    async def snake(self, ctx):
        if self.running:
            await ctx.reply("A game is already running.")
            return
        ...
        await ctx.reply("Welcome to Snake! Use reactions to control the snake.\n"
        ...
        while self.running:
            ...
            if not self.move():
                await ctx.reply("Game Over! Your snake collided with the wall or itself.")
                self.running = False  # Set the running flag to False to exit the loop
                break
